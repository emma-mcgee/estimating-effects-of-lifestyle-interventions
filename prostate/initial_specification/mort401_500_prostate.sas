...
Date Last Updated: November 21, 2022
...
%include '/udd/nhemc/survival_wcrf_bcpc/gformula4.0.sas';

libname results '/udd/nhemc/survival_wcrf_bcpc/final_results/prostate/primary';
libname gcomp '/udd/nhemc/survival_wcrf_bcpc/prostate';
...
* INTERVENTION 2 - All dietary components of recommendations;
%let interv2  =
    intno     = 2,
    intlabel  = 'All participants follow all WCRF/AICR dietary recommendations in all intervals',
    intcond   = (xcond = 0),
    nintvar   = 6,
    intvar1   = totfv,
    inttype1  = 2,
...
%let interv3  =
    intno     = 3,
    intlabel  = 'All participants follow the physical activity recommendation in all intervals',
    intcond   = (xcond = 0),
    nintvar   = 1,
    intvar1   = act,
    inttype1  = 2,
...
* INTERVENTION 4 - Fruits and vegetables;
%let interv4  =
    intno     = 4,
    intlabel  = 'All participants follow the fruit and vegetable recommendation in all intervals',
    intcond   = (xcond = 0),
    nintvar   = 1,
    intvar1   = totfv,
    inttype1  = 2,
...
* INTERVENTION 5 - Whole grains;
%let interv5  =
    intno     = 5,
    intlabel  = 'All participants follow the whole grains recommendation in all intervals',
    intcond   = (xcond = 0),
    nintvar   = 1,
    intvar1   = totgrn,
    inttype1  = 2,
...
* INTERVENTION 6 - Processed foods;
%let interv6  =
    intno     = 6,
    intlabel  = 'All participants follow the processed food recommendation in all intervals',
    intcond   = (xcond = 0),
    nintvar   = 1,
    intvar1   = junk,
    inttype1  = 2,
...
* INTERVENTION 7 - Red meat;
%let interv7  =
    intno     = 7,
    intlabel  = 'All participants follow the red meat recommendation in all intervals',
    intcond   = (xcond = 0),
    nintvar   = 1,
    intvar1   = totred,
    inttype1  = 2,
...
* INTERVENTION 8 - Processed meat;
%let interv8  =
    intno     = 8,
    intlabel  = 'All participants follow the processed meat recommendation in all intervals',
    intcond   = (xcond = 0),
    nintvar   = 1,
    intvar1   = totproc,
    inttype1  = 2,
...
* INTERVENTION 9 - Sugar sweetened beverages;
%let interv9  =
    intno     = 9,
    intlabel  = 'All participants follow the ssb recommendation in all intervals',
    intcond   = (xcond = 0),
    nintvar   = 1,
    intvar1   = sodajuice,
    inttype1  = 1,
...
* Check data;
proc freq data=bytimes;
tables period;
run;

proc freq data=bytimes;
tables period*(xcond baseage_1 baseage_2 baseage_3 fhxmi stage_1 gleason_1 gleason_2 psa_1  treat_1 treat_2 smkhx hbp_dia hbp chl dia);
run;

proc freq data=bytimes;
where period = 0;
tables baseage_1 baseage_2 baseage_3 baseage_4
	   lbmi_pre_1 lbmi_pre_2 lbmi_pre_3 lbmi_pre_4
       lcal_pre_1 lcal_pre_2 lcal_pre_3
       act_pre_1 act_pre_2 act_pre_3 act_pre_4
       totfv_pre_1 totfv_pre_2 totfv_pre_3 totfv_pre_4
       totgrn_pre_1 totgrn_pre_2 totgrn_pre_3 totgrn_pre_4
       junk_pre_1 junk_pre_2 junk_pre_3 junk_pre_4
       totred_pre_1 totred_pre_2 totred_pre_3 totred_pre_4
       totproc_pre_1 totproc_pre_2 totproc_pre_3 totproc_pre_4
       sodajuice_pre_1 sodajuice_pre_2 sodajuice_pre_3 sodajuice_pre_4
       totalc_pre_1 totalc_pre_2 totalc_pre_3 totalc_pre_4
       asp_b asp_b_1 asp_b_2 asp_b_3
       hbp_b hbp_chl_b dia_b hbp_chl_dia_b hbp_dia_b fhxca;
run;
...
proc means data= bytimes N nmiss;
where act=0 and period=0;
var act;
run;

proc means data= bytimes N nmiss;
where totfv=0 and period=0;
var totfv;
run;

proc means data= bytimes N nmiss;
where totgrn=0 and period=0;
var totgrn;
run;

proc means data= bytimes N nmiss;
where junk=0 and period=0;
var junk;
run;

proc means data= bytimes N nmiss;
where totred=0 and period=0;
var totred;
run;

proc means data= bytimes N nmiss;
where totproc=0 and period=0;
var totproc;
run;

proc means data= bytimes N nmiss;
where sodajuice=0 and period=0;
var sodajuice;
run;

proc means data= bytimes N nmiss;
where totalc=0 and period=0;
var totalc;
run;
