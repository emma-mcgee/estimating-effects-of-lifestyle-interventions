...
Purpose of Program: Run parametric g-formula for the the effect of a recommendation-based 
                    ...
                    Restrict to individuals who were drinkers prior to diagnosis
                    Then estimate the effect of stopping drinking vs. continuing drinking
                    on total cancer-related mortality
...
libname results '/udd/nhemc/survival_wcrf_bcpc/final_results/prostate/supp_alc_drinkers_can_mort';
...
%let interv1  =
    intno     = 1,
    intlabel  = 'All participants stop drinking',
    intcond   = (xcond = 0),
    nintvar   = 1,
    intvar1   = totalc,
    inttype1  = 1,
...
%let interv2  =
    intno     = 2,
    intlabel  = 'All participants keep drinking',
    intcond   = (xcond = 0),
    nintvar   = 1,
    intvar1   = totalc,
    inttype1  = 2,
...
* Code competing event;
* The competing event, dead_other, is 1 if a death from a cause other than cancer occurs in the interval i to i+1;
* Assumed temporal ordering is Ck (censor) -->  Dk (dead_other, competing event)  -->  Yk (dead_can, outcome event);
    dead_other = 0;
    if event = 1 and dead_can ne 1 then dead_other = 1;
    if censor = 1 then do ;
          dead_other = . ; 
          dead_can = . ;                          
    end;   
    if dead_other = 1 then do ;
        dead_can = . ;
    end;
