...
Purpose of Program: Run parametric g-formula for the the effect of a recommendation-based 
                    physical activity and dietary intervention on continuous fiber 20 years after baseline
...
data bytimes;
    set gcomp.bytimes_breast_fiber;
...
* Create an indicator of censoring or the event;
   ...
   * Setting up data format for continuous end of follow-up;
   if period < 9 then aofib = . ;        
   if censor_event = 1  then do ;
          aofib = . ;
    end;
 
 * Set aofib to missing if it was not actually measured in that qx_year;
   if qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014,
                  1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021) then aofib = .;
run;


* Check data;
proc freq data=bytimes;
tables censor event censor_event;
run;
...
proc means data= bytimes n nmiss min p10 p50 p90 max;
var aofib;
run;
...
fixedcov = baseage baseage_spl1
        ...
        aofib_pre_1 aofib_pre_2 aofib_pre_3
        smkhx
        hbp_chl_dia_b
		,
    ...
    cov4  = lcal,   cov4otype   = 3,  cov4ptype  = lag1cat,   cov4knots = 7.3777589  7.740664402,
                     ...
                     cov4wherenosim=(qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014, 2018, 2022, 2026, 2030, 2034,
                                                 1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021, 2025, 2029, 2033)), 
                     cov4nosimelsemacro=lcalmacro,
    cov5  = totalc,  cov5otype  = 4,  cov5ptype  = lag1spl,   cov5knots = 0.01  1 3, 
                     cov5etype  = cumavgnew 1, cov5eknots = 3,
                     ...
                     cov5wherenosim=(qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014, 2018, 2022, 2026, 2030, 2034,
                                                 1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021, 2025, 2029, 2033)), 
                     cov5nosimelsemacro=totalcmacro,
    cov6  = sodajuice, cov6otype  = 4,  cov6ptype  = lag1qdc, 
                     cov6etype  = cumavgnew 1, cov6eknots = 3,
                     ...
                     cov6wherenosim=(qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014, 2018, 2022, 2026, 2030, 2034,
                                                 1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021, 2025, 2029, 2033)), 
                     cov6nosimelsemacro=sodajuicemacro,
    cov7  = junk,   cov7otype   = 3,  cov7ptype  = lag1qdc,
                     cov7etype  = cumavgnew 1, cov7eknots = 3,
                     ...
                     cov7wherenosim=(qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014, 2018, 2022, 2026, 2030, 2034,
                                                 1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021, 2025, 2029, 2033)), 
                     cov7nosimelsemacro=junkmacro,
    cov8  = totproc, cov8otype  = 4,  cov8ptype  = lag1spl,   cov8knots = 0.1  1   4, 
                     cov8etype  = cumavgnew 1, cov8eknots = 3,
                     ...
                     cov8wherenosim=(qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014, 2018, 2022, 2026, 2030, 2034,
                                                 1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021, 2025, 2029, 2033)), 
                     cov8nosimelsemacro=totprocmacro,
    cov9  = totfv,  cov9otype   = 3,  cov9ptype  = lag1spl,   cov9knots = 2  5   7.9, 
                     cov9etype  = cumavgnew 1, cov9eknots = 3,
                     ...
                     cov9wherenosim=(qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014, 2018, 2022, 2026, 2030, 2034,
                                                 1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021, 2025, 2029, 2033)), 
                     cov9nosimelsemacro=totfvmacro,
    cov10  = totred, cov10otype   = 3,   cov10ptype  = lag1spl,   cov10knots = 0.8  3   7.5, 
                     cov10etype  = cumavgnew 1, cov10eknots = 3,
                     ...
                     cov10wherenosim=(qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014, 2018, 2022, 2026, 2030, 2034,
                                                 1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021, 2025, 2029, 2033)), 
                     cov10nosimelsemacro=totredmacro,   
    cov11  = totgrn, cov11otype   = 3,  cov11ptype  = lag1spl,   cov11knots = 0.4  1.4   3, 
                     cov11etype  = cumavgnew 1, cov11eknots = 3,
                     cov11wherem=(not (qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014, 2018, 2022, 2026, 2030, 2034,
                                                  1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021, 2025, 2029, 2033))), 
                     cov11wherenosim=(qx_year in (1988, 1992, 1996, 2000, 2004, 2008, 2012, 2014, 2018, 2022, 2026, 2030, 2034,
                                                 1989, 1993, 1997, 2001, 2005, 2009, 2013, 2017, 2021, 2025, 2029, 2033)), 
                     cov11nosimelsemacro=totgrnmacro,
    cov12  = act,    cov12otype   = 3,  cov12ptype  = lag1spl,  cov12knots = 0.5 7.5 44,
                     cov12etype  = cumavgnew 1, cov12eknots = 3,
                     cov12wherem=(not (qx_year in (1990, 2002, 2016, 2028,
                                                   1993, 1995, 1999, 2003, 2007, 2011, 2015, 2019, 2023, 2027, 2031, 2035))), 
                     cov12wherenosim=(qx_year in (1990, 2002, 2016, 2028,
                                                  1993, 1995, 1999, 2003, 2007, 2011, 2015, 2019, 2023, 2027, 2031, 2035)), 
                     cov12nosimelsemacro=actmacro,
