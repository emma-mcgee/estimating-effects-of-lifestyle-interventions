...
Purpose of Program: Run parametric g-formula for the effect of a recommendation-based 
                    ...
                    Additionally adjust for difference between physical activity and dietary variables pre vs. post diagnosis
...
data bytimes2;
   ...
   keep id lbmi_pre bmi_pre lcal_pre cal_pre
...
* Create variables for differences in diet and physical activity pre- vs. post-diagnosis;
   modact_difdx = modact_b - modact_predx;
   vigact_difdx = vigact_b - vigact_predx;
   act_difdx = act_b - act_predx;
   totfv_difdx = totfv_b - totfv_pre;
   totgrn_difdx = totgrn_b - totgrn_pre;
   junk_difdx = junk_b - junk_pre;
   totred_difdx = totred_b - totred_pre;
   totproc_difdx = totproc_b - totproc_pre;
   sodajuice_difdx = sodajuice_b - sodajuice_pre;
   totalc_difdx = totalc_b - totalc_pre;
   cal_difdx = cal_b - cal_pre;

   * Use cut points at the 25th, 50th, 75th percentiles;
   act_difdx_1 = 0; act_difdx_2 = 0; act_difdx_3=0; act_difdx_4=0;
   if act_difdx <-8.1 then act_difdx_1=1;
   if act_difdx>=-8.1 and act_difdx<0 then act_difdx_2=1;
   if act_difdx>=0 and act_difdx<7.0 then act_difdx_3=1;
   if act_difdx>=7.0 then act_difdx_4=1;

   totfv_difdx_1 = 0; totfv_difdx_2 = 0; totfv_difdx_3=0; totfv_difdx_4=0;
   if totfv_difdx <-1 then totfv_difdx_1=1;
   if totfv_difdx>=-1 and totfv_difdx<0.14 then totfv_difdx_2=1;
   if totfv_difdx>=0.14 and totfv_difdx<1.24 then totfv_difdx_3=1;
   if totfv_difdx>=1.24 then totfv_difdx_4=1;

   totgrn_difdx_1 = 0; totgrn_difdx_2 = 0; totgrn_difdx_3=0; totgrn_difdx_4=0;
   if totgrn_difdx <-0.36 then totgrn_difdx_1=1;
   if totgrn_difdx>=-0.36 and totgrn_difdx<0 then totgrn_difdx_2=1;
   if totgrn_difdx>=0 and totgrn_difdx<0.51 then totgrn_difdx_3=1;
   if totgrn_difdx>=0.51 then totgrn_difdx_4=1;

   junk_difdx_1 = 0; junk_difdx_2 = 0; junk_difdx_3=0; junk_difdx_4=0;
   if junk_difdx <-1.1 then junk_difdx_1=1;
   if junk_difdx>=-1.1 and junk_difdx<-0.22 then junk_difdx_2=1;
   if junk_difdx>=-0.22 and junk_difdx<0.56 then junk_difdx_3=1;
   if junk_difdx>=0.56 then junk_difdx_4=1;

   totred_difdx_1 = 0; totred_difdx_2 = 0; totred_difdx_3=0; totred_difdx_4=0;
   if totred_difdx <-1.79 then totred_difdx_1=1;
   if totred_difdx>=-1.79 and totred_difdx<0 then totred_difdx_2=1;
   if totred_difdx>=0 and totred_difdx<0.98 then totred_difdx_3=1;
   if totred_difdx>=0.98 then totred_difdx_4=1;

   totproc_difdx_1 = 0; totproc_difdx_2 = 0; totproc_difdx_3=0; totproc_difdx_4=0;
   if totproc_difdx <-0.50 then totproc_difdx_1=1;
   if totproc_difdx>=-0.50 and totproc_difdx<0 then totproc_difdx_2=1;
   if totproc_difdx>=0 and totproc_difdx<0.49 then totproc_difdx_3=1;
   if totproc_difdx>=0.49 then totproc_difdx_4=1;

   sodajuice_difdx_1 = 0; sodajuice_difdx_2 = 0; sodajuice_difdx_3=0; sodajuice_difdx_4=0;
   if sodajuice_difdx <-0.29 then sodajuice_difdx_1=1;
   if sodajuice_difdx>=-0.29 and sodajuice_difdx<0 then sodajuice_difdx_2=1;
   if sodajuice_difdx>=0 and sodajuice_difdx<0.29 then sodajuice_difdx_3=1;
   if sodajuice_difdx>=0.29 then sodajuice_difdx_4=1;

   totalc_difdx_1 = 0; totalc_difdx_2 = 0; totalc_difdx_3=0; totalc_difdx_4=0;
   if totalc_difdx <-0.07 then totalc_difdx_1=1;
   if totalc_difdx>=-0.07 and totalc_difdx<0 then totalc_difdx_2=1;
   if totalc_difdx>=0 and totalc_difdx<0.07 then totalc_difdx_3=1;
   if totalc_difdx>=0.07 then totalc_difdx_4=1;

run;


proc means data= bytimes p25 p50 p75;
where period=0;
var act totfv totgrn junk totred totproc sodajuice totalc
    act_difdx totfv_difdx totgrn_difdx junk_difdx totred_difdx totproc_difdx sodajuice_difdx totalc_difdx;
run;

proc freq;
where period=0;
tables  act_difdx_1 act_difdx_2 act_difdx_3 act_difdx_3
        totfv_difdx_1 totfv_difdx_2 totfv_difdx_3 totfv_difdx_4
        totgrn_difdx_1 totgrn_difdx_2 totgrn_difdx_3 totgrn_difdx_4
        junk_difdx_1 junk_difdx_2 junk_difdx_3 junk_difdx_4
        totred_difdx_1 totred_difdx_2 totred_difdx_3 totred_difdx_4
        totproc_difdx_1 totproc_difdx_2 totproc_difdx_3 totproc_difdx_4
        sodajuice_difdx_1 sodajuice_difdx_2 sodajuice_difdx_3 sodajuice_difdx_4
        totalc_difdx_1 totalc_difdx_2 totalc_difdx_3 totalc_difdx_4;
run;
...
fixedcov = baseage baseage_spl1
        ...
        smkhx
        hbp_chl_dia_b
        act_difdx_1 act_difdx_2 act_difdx_3
        totfv_difdx_1 totfv_difdx_2 totfv_difdx_3
        totgrn_difdx_1 totgrn_difdx_2 totgrn_difdx_3
        junk_difdx_1 junk_difdx_2 junk_difdx_3
        totred_difdx_1 totred_difdx_2 totred_difdx_3
        totproc_difdx_1 totproc_difdx_2 totproc_difdx_3
        sodajuice_difdx_1 sodajuice_difdx_2 sodajuice_difdx_3
        totalc_difdx_1 totalc_difdx_2 totalc_difdx_3
		,
    ...
    cov3  = lbmi,   cov3otype   = 3,  cov3ptype  = lag1cat,  cov3knots = 2.9177707 3.2188758 3.4011974,
    cov4  = lcal,   cov4otype   = 3,  cov4ptype  = lag1cat,   cov4knots = 7.3777589  7.740664402,
